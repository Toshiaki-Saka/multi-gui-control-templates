# build_and_run.ps1
# C++ コア (tdof_core.dll)、Qt6 フロントエンド、C# Avalonia をビルドし、
# 指定したフロントエンドを起動する。
#
# 使い方:
...
# 既存の CMakeCache があればジェネレータ指定は不要 (変更不可)、なければ VS を使用
...
if (-not $SkipBuild) {
    ...
    # tdof_core.dll の実際の場所を解決
    $coreDllCandidates = @(
        "$ROOT\core\build\$BuildType\tdof_core.dll",  # MSVC multi-config
        "$ROOT\core\build\tdof_core.dll"              # Ninja / MinGW
    )
    $coreDll = $coreDllCandidates | Where-Object { Test-Path $_ } | Select-Object -First 1
    if (-not $coreDll) { Die "tdof_core.dll が見つかりません" }
    Ok "core ビルド完了 → $coreDll"

    # csproj は ../../core/build/tdof_core.dll を参照するので直下にもコピー
    $coreFlat = "$ROOT\core\build\tdof_core.dll"
    if ($coreDll -ne $coreFlat) {
        Copy-Item $coreDll $coreFlat -Force
        Ok "tdof_core.dll → $coreFlat (csproj 用)"
    }
    ...
    if ($LaunchQt -and $Qt6Path) {
        ...
        # tdof_core.dll を Qt 実行ファイルと同じディレクトリにコピー
        $qtExeCandidates = @(
            "$ROOT\frontend_qt\build\$BuildType\tdof_qt.exe",
            "$ROOT\frontend_qt\build\tdof_qt.exe"
        )
        ...
        if ($qtExe -and $coreDll) {
            ...
            # windeployqt が拾わない vcpkg サードパーティ DLL を再帰的にコピー
            $vcpkgBinDir = "$Qt6Path\bin"
            if (Test-Path $vcpkgBinDir) {
                $deployDir = Split-Path $qtExe
                $copied = 0
                $changed = $true
                while ($changed) {
                    $changed = $false
                    $deployed = (Get-ChildItem "$deployDir\*.dll" -ErrorAction SilentlyContinue |
                                 Select-Object -ExpandProperty Name)
                    foreach ($dllFile in (Get-ChildItem "$deployDir\*.dll" -ErrorAction SilentlyContinue)) {
                        $raw = [System.IO.File]::ReadAllBytes($dllFile.FullName)
                        $txt = [System.Text.Encoding]::ASCII.GetString($raw)
                        $deps = [regex]::Matches($txt, '[A-Za-z0-9_\-]+\.dll') |
                                ForEach-Object { $_.Value } | Sort-Object -Unique
                        foreach ($dep in $deps) {
                            if ($deployed -icontains $dep) { continue }
                            if (Test-Path "C:\Windows\System32\$dep") { continue }
                            if (Test-Path "C:\Windows\System32\downlevel\$dep") { continue }
                            $src = "$vcpkgBinDir\$dep"
                            if (Test-Path $src) {
                                Copy-Item $src "$deployDir\$dep" -Force
                                $deployed += $dep
                                $copied++
                                $changed = $true
                            }
                ...
                if ($copied -gt 0) { Ok "vcpkg 依存 DLL $copied 件をコピーしました" }
            }
        }
        Ok "Qt6 ビルド完了"
    } elseif ($LaunchQt) {
    ...
    if ($LaunchCSharp) {
        Require "dotnet"
        $csprojDir = "$ROOT\frontend_avalonia\TdofAvalonia"
        ...
        # tdof_core.dll が出力ディレクトリにコピーされているか確認 (csproj 条件が効かない場合の保険)
        $avaloniaOut = "$csprojDir\bin\$BuildType\net8.0"
        if (-not (Test-Path "$avaloniaOut\tdof_core.dll")) {
            if ($coreDll) {
                Copy-Item $coreDll "$avaloniaOut\tdof_core.dll" -Force
                Ok "tdof_core.dll → $avaloniaOut (手動コピー)"
            }
        }
        Ok "Avalonia ビルド完了"
    }
...
# ════════════════════════════════════════════════════════════════════════════
#  tdof_core.dll パスを確定 (SkipBuild 時も使用)
# ════════════════════════════════════════════════════════════════════════════
$coreDllForLaunch = @(
    "$ROOT\core\build\$BuildType\tdof_core.dll",
    "$ROOT\core\build\tdof_core.dll"
...
if ($LaunchCSharp) {
    $csprojDir = "$ROOT\frontend_avalonia\TdofAvalonia"
    $avaloniaExe = @(
        "$csprojDir\bin\$BuildType\net8.0\TdofAvalonia.exe",
        "$csprojDir\bin\$BuildType\net8.0\win-x64\TdofAvalonia.exe"
    ) | Where-Object { Test-Path $_ } | Select-Object -First 1
