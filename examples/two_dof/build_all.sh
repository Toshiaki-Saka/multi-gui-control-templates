...
# Qt6 frontend. Avalonia and Python frontends have their own dependencies;
# see README.md.
...
cmake --build . --target tdof_core_smoke -j > /dev/null
./tdof_core_smoke
...
if command -v qmake6 >/dev/null 2>&1 || command -v qmake >/dev/null 2>&1; then
    ...
    cmake .. -DCMAKE_BUILD_TYPE=Release > /dev/null
    cmake --build . -j
    echo "    -> $ROOT/frontend_qt/build/tdof_qt"
else
...
echo "      cd $ROOT/frontend_avalonia/TdofAvalonia && dotnet run -c Release"
