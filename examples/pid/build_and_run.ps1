# build_and_run.ps1
# C++ コア (pid_core.dll)、Qt6 フロントエンド、C# Avalonia をビルドし、
# 指定したフロントエンドを起動する。
#
# 使い方:
...
switch ($Target) {
    ...
    default {
        if ($Target -ne 0) { Write-Host "ERROR: Target は 1(Qt6), 2(Avalonia), 3(Python) で指定してください。" -ForegroundColor Red; exit 1 }
...
# ── cmake 共通ヘルパー ────────────────────────────────────────────────────────
# CMakeCache.txt のソースパスが現在のパスと異なる場合はキャッシュを削除する
...
function Get-CmakeConfigArgs($buildDir, $prefixPath) {
    ...
    return ,$result   # leading comma preserves array type on return
}
...
if (-not $SkipBuild) {
    ...
    # pid_core.dll の実際の場所を解決
    $coreDllCandidates = @(
        "$ROOT\core\build\$BuildType\pid_core.dll",  # MSVC multi-config
        "$ROOT\core\build\pid_core.dll"              # Ninja / MinGW
    )
    $coreDll = $coreDllCandidates | Where-Object { Test-Path $_ } | Select-Object -First 1
    if (-not $coreDll) { Die "pid_core.dll が見つかりません" }
    Ok "core ビルド完了 → $coreDll"

    # csproj は ../../core/build/pid_core.dll を参照するので直下にもコピー
    $coreFlat = "$ROOT\core\build\pid_core.dll"
    if ($coreDll -ne $coreFlat) {
        Copy-Item $coreDll $coreFlat -Force
        Ok "pid_core.dll → $coreFlat (csproj 用)"
    }
    ...
    if ($LaunchQt -and $Qt6Path) {
        ...
        # pid_core.dll を Qt 実行ファイルと同じディレクトリにコピー
        $qtExeCandidates = @(
            "$ROOT\frontend_qt\build\$BuildType\pid_qt.exe",
            "$ROOT\frontend_qt\build\pid_qt.exe"
        )
        ...
        if ($qtExe -and $coreDll) {
            ...
            if ($windeployqt) {
                ...
                # windeployqt は Qt 以外の vcpkg 依存 DLL をコピーしない。
                # Qt6Core/Gui が必要とする既知のサードパーティ DLL を補完する。
                $vcpkgBin = "$Qt6Path\bin"
                if (Test-Path $vcpkgBin) {
                    $qtThirdPartyDlls = @(
                        "double-conversion.dll",  # Qt6Core
                        "pcre2-16.dll",            # Qt6Core
                        "z.dll",                   # Qt6Core/Gui (zlib)
                        "zstd.dll",                # Qt6Core
                        "harfbuzz.dll",            # Qt6Gui
                        "freetype.dll",            # Qt6Gui
                        "libpng16.dll",            # Qt6Gui
                    ...
                    if ($copied.Count -gt 0) {
                        Ok "vcpkg 依存 DLL を補完コピー: $($copied -join ', ')"
                    }
                }
            } else {
    ...
    if ($LaunchCSharp) {
        Require "dotnet"
        $csprojDir = "$ROOT\frontend_avalonia\PidAvalonia"
        ...
        # pid_core.dll が出力ディレクトリにコピーされているか確認 (csproj 条件が効かない場合の保険)
        $avaloniaOut = "$csprojDir\bin\$BuildType\net8.0"
        if (-not (Test-Path "$avaloniaOut\pid_core.dll")) {
            if ($coreDll) {
                Copy-Item $coreDll "$avaloniaOut\pid_core.dll" -Force
                Ok "pid_core.dll → $avaloniaOut (手動コピー)"
            }
        }
        Ok "Avalonia ビルド完了"
    }
...
# ════════════════════════════════════════════════════════════════════════════
#  pid_core.dll パスを確定 (SkipBuild 時も使用)
# ════════════════════════════════════════════════════════════════════════════
$coreDllForLaunch = @(
    "$ROOT\core\build\$BuildType\pid_core.dll",
    "$ROOT\core\build\pid_core.dll"
