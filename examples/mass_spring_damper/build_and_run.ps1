# build_and_run.ps1
# C++ コア (msd_core.dll)、Qt6 フロントエンド、C# Avalonia をビルドし、
# 指定したフロントエンドを起動する。
#
# 使い方:
...
# 以下は vcpkg ビルドの Qt6 で必要になる既知の DLL 一覧。
$qtThirdPartyDlls = @(
...
if (-not $SkipBuild) {
    ...
    $coreDllCandidates = @(
        "$ROOT\core\build\$BuildType\msd_core.dll",  # MSVC multi-config
        "$ROOT\core\build\msd_core.dll"              # Ninja / MinGW
    )
    $coreDll = $coreDllCandidates | Where-Object { Test-Path $_ } | Select-Object -First 1
    if (-not $coreDll) { Die "msd_core.dll が見つかりません" }
    Ok "core ビルド完了 → $coreDll"

    # csproj は ../../core/build/msd_core.dll を参照するので直下にもコピー
    $coreFlat = "$ROOT\core\build\msd_core.dll"
    if ($coreDll -ne $coreFlat) {
        Copy-Item $coreDll $coreFlat -Force
        Ok "msd_core.dll → $coreFlat (csproj 用)"
    }
    ...
    if ($LaunchCSharp) {
        Require "dotnet"
        $csprojDir = "$ROOT\frontend_avalonia\MsdAvalonia"
        ...
        # msd_core.dll が出力ディレクトリにコピーされているか確認 (csproj 条件の保険)
        $avaloniaOut = "$csprojDir\bin\$BuildType\net8.0"
        if ($coreDll -and -not (Test-Path "$avaloniaOut\msd_core.dll")) {
            Copy-Item $coreDll "$avaloniaOut\msd_core.dll" -Force
            Ok "msd_core.dll → $avaloniaOut (手動コピー)"
        }
        Ok "Avalonia ビルド完了"
    }
...
# ════════════════════════════════════════════════════════════════════════════
#  msd_core.dll パスを確定 (SkipBuild 時も使用)
# ════════════════════════════════════════════════════════════════════════════
$coreDllForLaunch = @(
    "$ROOT\core\build\$BuildType\msd_core.dll",
    "$ROOT\core\build\msd_core.dll"
...
if ($LaunchCSharp) {
    $csprojDir = "$ROOT\frontend_avalonia\MsdAvalonia"
    $avaloniaExe = @(
        "$csprojDir\bin\$BuildType\net8.0\MsdAvalonia.exe",
        "$csprojDir\bin\$BuildType\net8.0\win-x64\MsdAvalonia.exe"
    ) | Where-Object { Test-Path $_ } | Select-Object -First 1
